...
class ForestShrublandWithGrass : GlobGenerator(GlobType.FOREST_SHRUBLAND_WITH_GRASS) {
    companion object {
    ...
    override fun createLayers() {
        var cover: GenLayer = SelectWeightedLayer(1,
                SelectWeightedLayer.Entry(ForestShrublandWithGrass.LAYER_DIRT, 2),
                SelectWeightedLayer.Entry(ForestShrublandWithGrass.LAYER_GRASS, 8))
    ...
    override fun coverDecorate(primer: ChunkPrimer, random: Random, x: Int, z: Int) {
        ...
        this.iterate { localX: Int, localZ: Int ->
            ...
            if (grassLayer[bufferIndex] == 1 && random.nextInt(4) != 0) {
                primer.setBlockState(localX, y + 1, localZ, ForestShrublandWithGrass.TALL_GRASS)
            } else if (random.nextInt(8) == 0) {
                primer.setBlockState(localX, y + 1, localZ, ForestShrublandWithGrass.BUSH)
            }
        }
    }
    ...
    override fun getCover(x: Int, z: Int, random: Random) {
        this.coverLayer(this.coverBuffer, x, z, this.coverSelector) {
            when (it) {
                ForestShrublandWithGrass.LAYER_GRASS -> ForestShrublandWithGrass.GRASS
                ForestShrublandWithGrass.LAYER_DIRT -> ForestShrublandWithGrass.DIRT
                else -> ForestShrublandWithGrass.PODZOL
            }
        }
    }
}
