...
class EarthBiomeProvider(world: World, settings: EarthGenerationSettings) : BiomeProvider() {
    ...
    override fun findBiomePosition(x: Int, z: Int, radius: Int, biomes: List<Biome>, random: Random): BlockPos? {
        ...
        var iterations = 0
        val chunkMinX = (x - radius) shr 4
        val chunkMinZ = (z - radius) shr 4
        val chunkMaxX = (x + radius) shr 4
        val chunkMaxZ = (z + radius) shr 4
        for (chunkZ in chunkMinZ..chunkMaxZ) {
            for (chunkX in chunkMinX..chunkMaxX) {
                this.handler.getGlobRegion(this.globBuffer, chunkZ, chunkX)
                for (localZ in Math.max(chunkZ shl 4, z - radius)..Math.min(chunkZ + 1 shl 4, z + radius)) {
                    for (localX in Math.max(chunkX shl 4, x - radius)..Math.min(chunkX + 1 shl 4, x + radius)) {
                        val index = localX + localZ * 16
                        val biome = this.globBuffer[index].biome
                        if (biomes.contains(biome) && (pos == null || random.nextInt(iterations + 1) == 0)) {
                            pos = BlockPos(x + localX, 0, z + localZ)
                            ++iterations
                        }
                    }
                }
