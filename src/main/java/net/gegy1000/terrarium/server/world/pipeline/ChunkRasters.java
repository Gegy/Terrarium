...
public class ChunkRasters {
    private final RegionGenerationHandler regionHandler;
    private final ImmutableMap<RegionComponentType<?>, Data<?, ?>> chunkRasters;

    public ChunkRasters(RegionGenerationHandler regionHandler, TerrariumDataProvider dataSystem) {
    ...
    @SuppressWarnings("unchecked")
    public <T extends RasterData<V>, V> T get(RegionComponentType<T> componentType) {
    ...
    private class Data<T extends RasterData<V>, V> {
        ...
        public void fillRasterPartial(int originX, int originZ) {
            if (this.currentX != originX || this.currentZ != originZ) {
                this.raster = ChunkRasters.this.regionHandler.computePartialRaster(this.componentType, originX, originZ, 16, 16);
        ...
        public void fillRaster(int originX, int originZ) {
            if (this.currentX != originX || this.currentZ != originZ) {
                ChunkRasters.this.regionHandler.fillRaster(this.componentType, this.raster, originX, originZ);
                this.currentX = originX;
                this.currentZ = originZ;
            }
        }
