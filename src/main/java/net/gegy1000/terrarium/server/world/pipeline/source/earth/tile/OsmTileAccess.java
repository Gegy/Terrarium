package net.gegy1000.terrarium.server.world.pipeline.source.earth.tile;
...
public class OsmTileAccess implements OsmEntityProvider, TiledDataAccess {
    private final TLongObjectMap<OsmNode> nodes;
    private final TLongObjectMap<OsmWay> ways;

    public OsmTileAccess(TLongObjectMap<OsmNode> nodes, TLongObjectMap<OsmWay> ways) {
        this.nodes = nodes;
        this.ways = ways;
    }

    public OsmTileAccess(InMemoryMapDataSet data) {
        this(data.getNodes(), data.getWays());
    }

    public OsmTileAccess() {
    ...
    public OsmTileAccess merge(OsmTileAccess tile) {
