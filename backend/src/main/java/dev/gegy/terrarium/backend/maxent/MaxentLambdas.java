...
public class MaxentLambdas<T> {
    ...
    public static <T> MaxentLambdas<T> parse(final Reader reader) throws IOException, MaxentParseException {
        final List<Feature<T>> features = new ArrayList<>();
        final Object2FloatMap<String> fields = new Object2FloatOpenHashMap<>();

        final FeatureParser<T> featureParser = new FeatureParser<>(name -> Optional.of(new Feature.Parameter<>(name)));
    ...
    public Feature.Output<T> buildFeature(final MaxentOutputType outputType) {
        ...
        return new Feature.Output<>(features, linearPredictorNormalizer, densityNormalizer, entropy, outputType);
    }
}
